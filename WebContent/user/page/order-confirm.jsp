...
%>
        <div id="addresId1">
			<i class="user_choice">
				<input type="hidden" id="uid" name="uid" value="<%=address.getUid()%>">
				<input type="hidden" name="aid" value="<%=address.getRid()%>">
                <input type="radio" id="address" name="address" value="<%=address.getAddress() %>" checked />
            </i>
...
<%
	ArrayList<CartAndBook> list = (ArrayList<CartAndBook>)request.getAttribute("orderItemlist");
	...
	<ul class="item_detail">
               <li class="p_info">
                   <b><img src="../img/goods/<%=book.getIsbn() %>/detail1.jpg"/></b>
					<input type="hidden" id="book" name="book" value="<%=itemIds%>">
...
<script>
    ...
    $(".go_pay").click(function () {
    	var money = document.getElementsByTagName("payment")[0];
    	var rid = document.getElementById("address").value;
    	var uid = document.getElementById("uid").value;
    	var book = document.getElementById("book").value;
    	alert(money);
        location.href = "orderplacedServlet?rid="+rid+"&"+"uid="+uid+"&"+"book="+book;
    });
