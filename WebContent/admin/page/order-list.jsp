      ...
      <ul class="sidebar-menu" data-widget="tree">
        ...
        <li class="treeview">
          ...
          <ul class="treeview-menu">
            ...
            <li><a href='orderListAllServlet1?sta=待处理'>待处理</a></li>
            <li><a href='orderListAllServlet1?sta=处理中'>处理中</a></li>
            <li><a href='orderListAllServlet1?sta=已发货'>已发货</a></li>
            <li><a href='orderListAllServlet1?sta=已交付'>已交付</a></li>
          </ul>
        </li>
        <li class="treeview">
                ...
                <div class="box box-primary">
                  <div class="box-body">
                    <table id="my_order" class="table table-bordered table-striped">
                      ...
                      <tbody>
                       <%
    					ArrayList<OrderStatus> books = (ArrayList<OrderStatus>)request.getAttribute("allbooks");
    					for(int i=0;i<books.size();i++){
    						OrderStatus book = books.get(i);
    					%>
                      <tr>
                        <td><%=book.getOid() %></td>
                        <td><span class="label bg-yellow"><%=book.getSta() %></span></td>
                        <td><%=book.getPlaced() %></td>
                        <td><%=book.getPayment() %></td>
                        <td><a href="order-detail?uid=<%=book.getUid()%>&oid=<%=book.getOid()%>&aid=<%=book.getAid()%>">查看</a>丨<a href="order-process?uid=<%=book.getUid()%>&oid=<%=book.getOid()%>&aid=<%=book.getAid()%>">处理</a></td>
                      </tr>
                      <%
      }
      %>
